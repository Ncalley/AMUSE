 ...
 * @version $Id: CommaSelection.java 241 2018-07-26 12:35:24Z frederik-h $
 */
public class CommaSelection extends AbstractSelection {

	public CommaSelection(EvolutionaryStrategy correspondingES) {
	...
	public int replaceParentPopulation() {
		...
		individualsToSort = new ArrayList<Integer>(correspondingES.offspringPopSize);
		for(int i=0;i<correspondingES.offspringPopSize;i++) {
			individualsToSort.add(i+correspondingES.popSize);
		}
		...
		for(int i=0;i<correspondingES.popSize;i++) {
			...
			for(int j=1;j<individualsToSort.size();j++) {
				if(correspondingES.isMinimizingFitness) {
					// TODO hier only single-objective optimization is used! 
